-- Published price change for JuntAPP + website.
ALTER TABLE public.juntas DROP CONSTRAINT IF EXISTS juntas_subscription_price_check;
UPDATE public.juntas SET subscription_price = CASE WHEN whatsapp_addon THEN 30980 ELSE 22990 END
WHERE subscription_plan = 'juntapp_web';
ALTER TABLE public.juntas ADD CONSTRAINT juntas_subscription_price_check
  CHECK (subscription_price IN (9990, 14990, 15000, 17980, 22980, 22990, 30980));
