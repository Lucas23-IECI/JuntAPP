...
export type ChileRegion = keyof typeof CHILE_LOCATIONS;

export const CHILE_REGIONS = Object.keys(CHILE_LOCATIONS) as ChileRegion[];

export function getCommunes(region: string): readonly string[] {
  return region in CHILE_LOCATIONS ? CHILE_LOCATIONS[region as ChileRegion] : [];
}

export function isValidChileLocation(region: string, comuna: string): boolean {
  return getCommunes(region).includes(comuna as never);
}
