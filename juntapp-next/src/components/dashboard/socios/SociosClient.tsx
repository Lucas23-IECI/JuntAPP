...
export default function SociosClient({ socios, currentProfile, junta, paidByMercadoPagoHouseholdIds, applications }: { socios: Profile[]; currentProfile: Profile; junta: Junta; paidByMercadoPagoHouseholdIds: string[]; applications: MembershipApplication[] }) {
  ...
  return (
    <section className="app-view active" id="view-socios">
      ...
      {isDirigente ? <>
        <div className="board-position-panel shadow-box"><div><span className="view-subtitle">Ingreso controlado de socios</span><h3 className="card-title">Código de la junta: {junta.invite_code}</h3><p>El enlace permite solicitar ingreso a {junta.name}. Secretaría debe aprobar cada solicitud antes de habilitar la cuenta; toda la directiva recibe copia.</p></div><div className="board-position-control"><button type="button" className="btn btn-primary" onClick={() => void copyRegistrationLink()}>Copiar enlace de solicitud</button>{inviteCodeMessage && <span className="form-help">{inviteCodeMessage}</span>}</div></div>
        <div className="card shadow-box mb-24"><div className="card-header"><div><span className="view-subtitle">Bandeja de Secretaría</span><h3 className="card-title">Solicitudes de ingreso ({applications.length})</h3></div></div><p className="card-subtitle">La directiva puede revisar las cartas. Solo Secretaría puede aceptar o rechazar.</p><div className="past-polls-list">{applications.map((application) => <article className="past-poll-row" key={application.id}><div className="past-poll-info"><h4 className="past-poll-title">{application.name}</h4><p className="past-poll-desc">RUT {formatRUT(application.rut)} · {application.address} · {application.email}</p><span className="form-help">Carta: {application.letter_delivery_status === 'sent' ? 'correo enviado y copia interna' : application.letter_delivery_status === 'failed' ? 'copia interna; correo no entregado' : 'copia interna'}</span></div>{currentProfile.board_position === 'secretario' ? <div className="view-actions"><button className="btn btn-sm btn-secondary danger-action" disabled={applicationLoading === application.id} onClick={() => void decideApplication(application, 'reject')}>Rechazar</button><button className="btn btn-sm btn-primary" disabled={applicationLoading === application.id} onClick={() => void decideApplication(application, 'approve')}>{applicationLoading === application.id ? 'Procesando…' : 'Aceptar e invitar'}</button></div> : <span className="status-badge pendiente">Pendiente de Secretaría</span>}</article>)}{!applications.length && <p className="empty-state">No hay solicitudes pendientes.</p>}</div>{applicationError && <p className="form-error">{applicationError}</p>}</div>
